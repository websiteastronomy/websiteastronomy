...
async function runNotificationSafely(label: string, task: () => Promise<void>) {
  try {
    ...
    console.error(`[observations] notification failure: ${label}`, error);
  }
}

async function getObservationAccess(userId: string) {
  ...
  const canCoreReview =
    isAdmin ||
    (await hasPermission(userId, "approve_actions")) ||
    (await hasPermission(userId, "manage_projects"));
...
async function getAssignedReviewerIds(observerId: string) {
  ...
  const reviewerFlags = await Promise.all(
    allUsers.map(async (user) => {
      const access = await getObservationAccess(user.id);
      return {
        id: user.id,
        canReview: user.id !== observerId && access.canCoreReview,
      };
    })
  );
...
async function notifyObservationSubmitted(params: {
  ...
  await runNotificationSafely("observation_submitted", async () => {
...
async function notifyAdminCoreApproved(params: {
  ...
  const adminUserIds = await getAdminUserIds();
  await runNotificationSafely("observation_core_approved", async () => {
    await createNotificationsForUsers(
...
async function notifyObserverDecision(params: {
  ...
  await runNotificationSafely("observation_observer_decision", async () => {
...
function getReviewConsensusThreshold(assignedReviewers: unknown) {
  const reviewerCount = Array.isArray(assignedReviewers)
    ? assignedReviewers.filter(Boolean).length
    : 0;
...
export async function editObservationAction(id: string, data: any, isDraft: boolean) {
  ...
  if (obs.observerId !== session.user.id && !access.isAdmin) {
  ...
  revalidatePath("/portal/observations");
  revalidatePath("/core/observations");
  if (access.isAdmin) revalidatePath("/admin");
  return { success: true };
}
...
export async function processCoreVoteAction(id: string, vote: "approve" | "reject", reason?: string) {
  ...
  const access = await getObservationAccess(session.user.id);
  if (!access.canCoreReview) throw new Error("Forbidden: Core permission required");
  ...
  const assignedReviewers = Array.isArray(obs.assignedReviewers)
    ...
    : [];
  const consensusThreshold = getReviewConsensusThreshold(assignedReviewers);
  if (
  ...
  // Evaluate state advancement based on the assigned reviewer pool.
  let newStatus: string = obs.status === "Submitted" ? "Under_Review" : obs.status;
  let rejectionReasonStr = obs.rejectionReason;

  if (approvals.length >= consensusThreshold) {
    newStatus = "Core_Approved";
  } else if (rejections.length >= consensusThreshold) {
    newStatus = "Rejected";
...
export async function adminFinalizeObservationAction(id: string, decision: "approve" | "reject", reason?: string) {
  ...
  const access = await getObservationAccess(session.user.id);
  if (!access.isAdmin) throw new Error("Forbidden: Admin required");
...
export async function reportObservationAction(id: string, reason: string) {
  ...
  if (session?.user?.id) {
    const [existingReport] = await db
      .select({ id: observation_reports.id })
      .from(observation_reports)
      .where(
        and(
          eq(observation_reports.observationId, id),
          eq(observation_reports.reporterId, session.user.id)
        )
      )
      .limit(1);

    if (existingReport) {
      throw new Error("You have already reported this observation.");
    }
  }

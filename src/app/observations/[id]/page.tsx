...
export default function ObservationDetail() {
  ...
  const handleReport = async () => {
    if (!reportReason.trim()) return setFeedback({ type: "error", message: "Please provide a reason for reporting this observation." });

    setIsReporting(true);
    setFeedback(null);
    try {
      const res = await reportObservationAction(id, reportReason.trim());
      setFeedback({
        type: "success",
        message: res.flagged
      ...
      if (res.flagged) setTimeout(() => router.push("/observations"), 1200);
    } catch (err) {
      setFeedback({ type: "error", message: "Failed to send report." });
    } finally {
  ...
  const formatDate = (dateString: string) => {
    ...
    return formatDateTimeStable(dateString);
  };
                ...
                {showReportForm ? (
                  <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                    <textarea
                      ...
                      placeholder="Please provide a reason for reporting this observation..."
                      rows={4}
                      style={{ width: "100%", background: "rgba(15,22,40,0.45)", border: "1px solid var(--border-subtle)", borderRadius: "8px", padding: "0.8rem", color: "var(--text-primary)", fontSize: "0.82rem", resize: "vertical", fontFamily: "inherit" }}
                    />
                    <div style={{ display: "flex", gap: "0.75rem" }}>
                      <button 
                        ...
                        style={{ flex: 1, fontSize: "0.85rem", borderColor: "rgba(239, 68, 68, 0.4)", color: "#fca5a5" }}
                      >
                      ...
                      <button
                        ...
                        disabled={isReporting}
                        style={{ flex: 1, fontSize: "0.85rem", background: "transparent" }}
                      >
                        Cancel
                      </button>
                    </div>

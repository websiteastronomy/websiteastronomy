...
export async function POST(req: Request) {
  try {
    ...
    const intent: UploadIntent = {
      category: String(formData.get("category") || "general") as UploadIntent["category"],
      fileName: String(formData.get("fileName") || file.name || ""),
      fileType: String(formData.get("fileType") || file.type || "application/octet-stream"),
      fileSize: Number(formData.get("fileSize") || file.size || 0),
      entityId: (formData.get("entityId") as string | null) || null,
      projectId: (formData.get("projectId") as string | null) || null,
      isGlobal: String(formData.get("isGlobal") || "false") === "true",
      isPublic: String(formData.get("isPublic") || "true") !== "false",
    };
    ...
    const status =
      message.startsWith("Unauthorized")
        ...
        : message.startsWith("File too large") ||
            ...
            message.includes("Missing file metadata") ||
            message.includes("Missing upload file")
          ? 400
          : 500;

...
export default function GalleryManager() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!file && !editingMedia?.imageUrl) {
        setFeedback({ type: 'error', message: 'Image upload is required.' });
        setIsSaving(false);
        return;
      }
      ...
      if (editingMedia?.id) {
        ...
        setFeedback({ type: 'success', message: 'Uploaded successfully!' });
      }
      setShowForm(false);
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      setFeedback({ type: 'success', message: 'Media entry deleted.' });
        ...
        {media.map((med) => (
          <div key={med.id} style={{ ...rowStyle, padding: '1.2rem', alignItems: 'flex-start', borderLeft: med.isFeatured ? "3px solid #34d399" : "3px solid transparent" }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', alignItems: 'flex-end', marginLeft: "1rem" }}>
              ...
              <button 
                onClick={() => setPendingDeleteId((current) => current === med.id ? null : med.id)}
              ...
              >
                {pendingDeleteId === med.id ? 'Cancel' : 'Delete'}
              </button>
              {pendingDeleteId === med.id && (
                <button onClick={() => handleDelete(med.id)} style={{ background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.45)', color: '#fecaca', padding: '0.3rem 0.6rem', borderRadius: '4px', cursor: 'pointer', fontSize: '0.75rem', fontFamily: 'inherit', minWidth: '80px' }}>
                  Confirm
                </button>
              )}
            </div>

...
export default function ArticlesManager() {
  ...
  const loadSnapshot = async () => {
    ...
    try {
      ...
      console.error(error);
      window.alert("Failed to load article queues.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const openEditor = async (article?: ManagedArticle | null) => {
    ...
    try {
      ...
      console.error(error);
      window.alert(error instanceof Error ? error.message : "Failed to open article editor.");
    }
  };
  ...
  const handleUpdateFact = () => {
    ...
    window.alert("Daily fact updated in local site settings.");
  };
                      ...
                      <button
                        ...
                        onClick={async () => {
                          try {
                            ...
                            console.error(error);
                            window.alert(error instanceof Error ? error.message : "Rollback failed.");
                          }
                        }}
                      >
                        Roll Back
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", gap: "1rem", flexWrap: "wrap" }}>
                    ...
                    <div style={{ display: "flex", gap: "0.7rem", flexWrap: "wrap" }}>
                      {request.coreStatus === "pending" ? (
                        <>
                          <button className="btn-secondary" style={{ background: "transparent", cursor: "pointer", fontSize: "0.75rem" }} onClick={async () => { await reviewArticleEditRequestAction(request.id, "approve"); await loadSnapshot(); }}>
                            Core Approve
                          </button>
                          <button className="btn-secondary" style={{ background: "transparent", cursor: "pointer", fontSize: "0.75rem", borderColor: "rgba(239,68,68,0.35)", color: "#ef4444" }} onClick={async () => { await reviewArticleEditRequestAction(request.id, "reject"); await loadSnapshot(); }}>
                            Core Reject
                          </button>
                        </>
                      ) : null}
                      {canPublish && request.coreStatus === "approved" && request.adminStatus === "pending" ? (
                        <>
                          <button className="btn-primary" style={{ cursor: "pointer", fontSize: "0.75rem" }} onClick={async () => { await finalizeArticleEditRequestAction(request.id, "approve"); await loadSnapshot(); }}>
                            Merge Changes
                          </button>
                          <button className="btn-secondary" style={{ background: "transparent", cursor: "pointer", fontSize: "0.75rem", borderColor: "rgba(239,68,68,0.35)", color: "#ef4444" }} onClick={async () => { await finalizeArticleEditRequestAction(request.id, "reject"); await loadSnapshot(); }}>
                            Admin Reject
                          </button>
                        </>
                      ) : null}
            ...
            <div key={article.id} style={{ ...rowStyle, padding: "1.2rem", alignItems: "flex-start" }}>
              ...
              <div style={{ display: "flex", gap: "0.6rem", flexWrap: "wrap", justifyContent: "flex-end" }}>
                ...
                {canReview && article.status === "under_review" && !article.coreApproved ? (
                  <>
                    <button className="btn-secondary" style={{ fontSize: "0.75rem", background: "transparent", cursor: "pointer" }} onClick={async () => { await reviewArticleAction(article.id, "approve"); await loadSnapshot(); }}>
                      Core Approve
                    </button>
                    <button className="btn-secondary" style={{ fontSize: "0.75rem", background: "transparent", cursor: "pointer", borderColor: "rgba(239,68,68,0.35)", color: "#ef4444" }} onClick={async () => { await reviewArticleAction(article.id, "reject"); await loadSnapshot(); }}>
                      Reject
                    </button>
                  </>
                ) : null}
                {canPublish && article.status === "under_review" && article.coreApproved ? (
                  <button className="btn-primary" style={{ fontSize: "0.75rem", cursor: "pointer" }} onClick={async () => { await publishArticleAction(article.id); await loadSnapshot(); }}>
                    Publish
                  </button>
                ) : null}
                {canPublish && article.status === "published" ? (
                  <button className="btn-secondary" style={{ fontSize: "0.75rem", background: "transparent", cursor: "pointer" }} onClick={async () => { await setArticleFeaturedAction(article.id, !article.isFeatured); await loadSnapshot(); }}>
                    {article.isFeatured ? "Unfeature" : "Feature"}
                  </button>
                ) : null}
                {canPublish ? (
                  <button className="btn-secondary" style={{ fontSize: "0.75rem", background: "transparent", cursor: "pointer", borderColor: "rgba(239,68,68,0.35)", color: "#ef4444" }} onClick={async () => { if (window.confirm(`Soft delete "${article.title}"?`)) { await softDeleteArticleAction(article.id); await loadSnapshot(); } }}>
                    Delete
                  </button>
                ) : null}
              </div>

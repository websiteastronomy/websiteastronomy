    ...
    <>
      ...
      >
        ...
        <div className="site-navbar-desktop" style={{ display: 'flex', gap: '1.8rem', alignItems: 'center' }}>
          ...
          <div className="nav-dropdown">
            <span className="nav-link" style={{ cursor: 'pointer', color: isActive('/projects') || isActive('/observations') || isActive('/events') || isActive('/outreach') || isActive('/documentation') || isActive('/education') ? 'var(--gold-light)' : undefined }}>
              Explore ▾
            </span>
            <div className="dropdown-menu">
              ...
              {features.observationsEnabled && <Link href="/observations" className="dropdown-item" style={isActive('/observations') ? { color: 'var(--gold-light)' } : {}} prefetch={false}>Observations</Link>}
              {features.eventsEnabled && <Link href="/events" className="dropdown-item" style={isActive('/events') ? { color: 'var(--gold-light)' } : {}} prefetch={false}>Events</Link>}
              ...
              <Link href="/education" className="dropdown-item" style={isActive('/education') ? { color: 'var(--gold-light)' } : {}} prefetch={false}>Education</Link>
            </div>
          </div>
          ...
          {user ? (
            <Link href="/app" className="btn-primary" style={{ padding: '0.5rem 1.4rem', fontSize: '0.8rem', marginLeft: '0.5rem' }} prefetch={false}>
              Dashboard
            </Link>
          ) : (
            <Link href="/login" className="btn-primary" style={{ padding: '0.5rem 1.4rem', fontSize: '0.8rem', marginLeft: '0.5rem' }} prefetch={false}>
              Login
            </Link>
          )}
        </div>
      ...
      >
        ...
        <div className="site-mobile-nav-links">
          ...
          {user ? (
            <Link href="/app" className="btn-primary mobile-nav-cta" style={{ width: '100%', textAlign: 'center', marginTop: '1rem', display: 'block', boxSizing: 'border-box' }} onClick={closeMobileMenu}>Dashboard</Link>
          ) : (
            <Link href="/login" className="btn-primary mobile-nav-cta" style={{ width: '100%', textAlign: 'center', marginTop: '1rem', display: 'block', boxSizing: 'border-box' }} onClick={closeMobileMenu}>Login</Link>
          )}
        </div>
      </div>
    </>
